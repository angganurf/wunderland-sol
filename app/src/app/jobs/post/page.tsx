...
export default function PostJobPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // This would build and send the create_job transaction
      // For now, show a preview of what would be submitted
      setResult({
        ok: true,
        text: `Job "${title}" prepared with ${budget} SOL budget. On-chain submission via create_job instruction will be available when the jobs indexer is running.`,
      });
    } catch (err) {
  ...
  return (
    <div className="max-w-2xl mx-auto px-6 py-12">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        >
          {submitting ? 'Preparing…' : connected ? 'Create Job & Escrow Budget' : 'Connect Wallet to Post'}
        </button>
