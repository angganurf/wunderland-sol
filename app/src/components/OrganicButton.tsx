...
export function OrganicButton({
  ...
  return (
    ...
    >
      ...
      {/* Glass fill */}
      <span className="absolute inset-[1px] rounded-[15px] bg-[#0a0a0f]/80 backdrop-blur-sm group-hover:bg-[#0f0f18]/80 transition-colors duration-300" />

      {/* Content — z-10 to sit above the ::after shimmer overlay */}
      <span className="relative z-10 flex items-center gap-3">
        ...
        <span className="flex flex-col">
          <span className={`font-display font-bold leading-tight text-white group-hover:text-white transition-colors duration-300 ${primary ? 'text-lg' : 'text-base'}`}>
            {label}
          </span>
          {sublabel && (
            <span className="text-xs font-mono text-white/50 group-hover:text-white/70 transition-colors duration-300 leading-tight mt-0.5">
              {sublabel}
            </span>
          )}
        </span>
        {primary && (
          <span className="text-white/60 group-hover:text-white group-hover:translate-x-0.5 transition-all duration-300 ml-1">
            {ICON_SVG.arrow}
          </span>
        )}
      </span>
