...
const isNodeTestRun = (): boolean => process.argv.includes('--test') || process.env.NODE_ENV === 'test';
...
export class WunderlandVectorMemoryService implements OnModuleDestroy {
  ...
  async onModuleDestroy(): Promise<void> {
    try {
      ...
      this.logger.warn(`Failed to shutdown RetrievalAugmentor: ${String((err as any)?.message ?? err)}`);
    }
    try {
      await this.vectorStoreManager?.shutdownAllProviders();
    } catch (err) {
      this.logger.warn(`Failed to shutdown VectorStoreManager: ${String((err as any)?.message ?? err)}`);
    }
    try {
      await this.embeddingManager?.shutdown();
    } catch (err) {
      this.logger.warn(`Failed to shutdown EmbeddingManager: ${String((err as any)?.message ?? err)}`);
    }
  }
  ...
  private async ensureInitialized(): Promise<void> {
    ...
    this.initPromise = (async () => {
      try {
        ...
        this.disabledReason =
          (err as any)?.message ? String((err as any).message) : 'Vector memory initialization failed.';
  ...
  private buildProviderConfigs(): ProviderConfigEntry[] {
    ...
    const ollamaConfiguredBaseURL =
      coerceOllamaBaseURL(process.env.OLLAMA_BASE_URL) ?? coerceOllamaBaseURL(process.env.OLLAMA_HOST);
  ...
  private buildEmbeddingCandidates(providerManager: AIModelProviderManager): Array<{ providerId: string; modelId: string }> {
    ...
    if (explicitProvider) {
      ...
      if (providerId === 'ollama') {
        candidates.push({
          providerId,
          modelId: process.env.OLLAMA_EMBED_MODEL?.trim() || 'nomic-embed-text',
        });
        return candidates;
      }
    ...
    if (providerManager.getProvider('ollama')) {
      const ollamaModel = process.env.OLLAMA_EMBED_MODEL?.trim() || 'nomic-embed-text';
  ...
  private async probeEmbeddingDimension(providerManager: AIModelProviderManager): Promise<EmbedProbeResult> {
    ...
    for (const candidate of candidates) {
      ...
      try {
        ...
        errors.push(`${candidate.providerId}/${candidate.modelId}: ${(err as any)?.message ?? String(err)}`);
      }
    }
  ...
  private buildVectorStoreConfig(dimension: number): VectorStoreManagerConfig {
    ...
    return {
      ...
      defaultProviderId: 'wunderland-memory-sql',
      defaultEmbeddingDimension: dimension,
    };
  }
  ...
  private async _doInitialize(): Promise<void> {
    ...
    this.logger.log(`Using embeddings: provider='${embed.providerId}' model='${embed.modelId}' dim=${embed.dimension}`);

    const embeddingConfig: EmbeddingManagerConfig = {
      embeddingModels: [{ providerId: embed.providerId, modelId: embed.modelId, dimension: embed.dimension, isDefault: true }],
    ...
    await this.vectorStoreManager.initialize(
      this.buildVectorStoreConfig(embed.dimension),
      [
        {
          dataSourceId: this.dataSourceId,
          displayName: 'Wunderland Seed Memory',
          vectorStoreProviderId: 'wunderland-memory-sql',
    ...
    // Ensure the collection exists for the SQL vector store.
    const store = this.vectorStoreManager.getProvider('wunderland-memory-sql');
    if (store.collectionExists && store.createCollection) {
      const exists = await store.collectionExists(this.collectionName);
      if (!exists) {
        await store.createCollection(this.collectionName, embed.dimension, { similarityMetric: 'cosine' });
      }
    }
    ...
    if (cohereKey) {
      rerankerProviders.push({ providerId: 'cohere', apiKey: cohereKey, defaultModelId: 'rerank-v3.5' } as any);
    }
  ...
  private getDefaultQueryOptions(): Pick<RagRetrievalOptions, 'strategy' | 'strategyParams' | 'rerankerConfig'> {
    ...
    const hybridAlpha = Number.isFinite(hybridAlphaEnv) ? Math.max(0, Math.min(1, hybridAlphaEnv)) : 0.7;
